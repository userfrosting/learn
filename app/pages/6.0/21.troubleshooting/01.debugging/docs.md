...
To solve this problem, UserFrosting's [client-side components](/ui-theming/components-overview) can automatically replace the current window's contents with the contents of the response body when an error (4xx or 5xx) code is returned during an AJAX request.
