...
description: Master pluralization, nested keys, special handles, and complex translation scenarios
---

The translator offers powerful features for handling complex translation scenarios, including pluralization rules, nested keys, special handles, and reference placeholders. This page covers advanced techniques for building sophisticated multilingual applications.

## Pluralization

The translator automatically handles pluralization of strings. For a given language, there is a grammatical rule on how to change words depending on the number qualifying the word. Different languages can have different rules. For example, when dealing with an absence of cars, in English you say "**no carS**" (note the **s**), while in French you say **Aucune voiture** (note the **absence** of the "s" at the end of "voiture").

The rules associated with a particular language that are used by UserFrosting are based on [Mozilla plural rules](https://developer.mozilla.org/en-US/docs/Mozilla/Localization/Localization_and_Plurals). For example, the **English** locale uses rule #1, while the **French** locale uses rule #2. The plural rule a locale will apply is defined in [its configuration file](/i18n/custom-locale#plural-rule).

In the dictionary, message keys that support a plural form will have an array as localized messages instead of a single string. This array uses the **rules form** as the key. The right plural form to use is determined by the placeholder value passed as the second parameter of the `translate` function:
...
> [!TIP]
> Note that the `plural` placeholder can be overwritten using [the `@PLURAL` special handle](#plural-special-handle).

In this example, you can see that `0` is used as a special form to display `No hungry cats` instead of `0 hungry cats` to create more user-friendly messages.
...
### Multiple plurals in a string

If a localized string contains more than one plural, for example **X guest(s) and X friend(s) currently online**, you can apply the plural rule to both `guest` and `friends` by [nesting](#nested-keys) the `ONLINE_GUEST` and `ONLINE_FRIEND` keys into `ONLINE_USERS`:
...
> [!NOTE]
> Nested translations can be used when faced with long sentences using multiple sub-strings or plural forms, but those should be avoided when possible. Shorter or multiple sentences should be preferred instead. Special [handles](#the-placeholder) can also be useful in those cases.

### `@PLURAL` special handle

The default `plural` placeholder can be overwritten by the `@PLURAL` handle in the language files. This may be useful if you pass an existing array to the translate function, or when using multiple placeholders.
...
Nested keys can be accessed using _dot notation_, e.g., `translate('COLOR.BLACK')` will return `black`. Nested keys are also useful when multiple *master keys* share the same context. For example, let's consider:
...
In the future, if `METHOD` were to change to something else, or if you want to add a `METHOD.CHOOSE` key, it would make it easier to navigate your dictionary code.

### Nested keys and plural forms

Of course, nested keys and plural rules can live together inside the same master key. For example:
...
This behavior is useful when you don't want to translate the same word over and over again. It can also be used in complex translations with plural forms, but be careful when using this with plurals as the plural value is passed to all child translations and can cause conflicts (See [Example of a complex translation](#example-of-a-complex-translation)).
...
In this example, `{{&MY_CATS}}` gets replaced with the value of `MY_CATS` to create `I love my {{plural}} cats`. Since there are 3 cats, rule #2 is selected. So the string becomes `I love my 3 cats`.
...
To make it work in both languages, we need to call the translation and pass the color key as a placeholder using the `&` prefix:
...
"MY_CATS" => [
    0 => "Je n'ai pas de chats",
    1 => "J'ai un chat {{color}}",
    2 => "J'ai {{plural}} chats {{color}}s"
];
```
