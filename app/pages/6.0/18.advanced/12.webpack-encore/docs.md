...
>
> Please see:
> - [Asset Management Introduction](/assets-vite/introduction) for Vite-based asset management
> - [Getting Started with Vite](/assets-vite/getting-started) for configuration guide
> - [Migration Guide](/assets-vite/advanced/migration) if you need to migrate from Webpack Encore to Vite
...
Modern frameworks also complicate things. For example, [Vue.js](https://vuejs.org), [React](https://react.dev), [Sass](https://sass-lang.com) and [Less](https://lesscss.org) require a bundling or compilation step. Finally, third-party dependencies can be managed through npm, similar to how Composer manages PHP dependencies, and are not known to the UserFrosting PHP backend.

For these reasons, UserFrosting uses **Symfony's [Webpack Encore](https://github.com/symfony/webpack-encore)** to handle all frontend-related tasks. Webpack Encore is a simpler way to integrate [Webpack](https://webpack.js.org). It provides a clean & powerful API for bundling JavaScript modules, pre-processing CSS & JS and compiling and minifying assets, and provides the necessary integration in the Twig template system.

Compiling assets through Webpack Encore basically does the following:

1. It copies static assets from individual Sprinkles to the public web directory, so that they can be served directly by the web server instead of having to go through UserFrosting;
...
6. In production, it enables versioning of each asset file, to better manage caching by the browser;
7. And so much more.
...
Where Composer has its `composer.json` file to define the project dependencies, npm has the `package.json` file. The default UserFrosting `package.json` looks like this:
...
**Dependencies** are your app's frontend dependencies, which will eventually be passed to the browser. For example, _jQuery_ can be a dependency, as it will be used by the browser. **devDependencies** are CLI dependencies. They are required to run the building process. Finally, **scripts** are npm command aliases.

In the example above, you can see both the *Admin Sprinkle* and *AdminLTE Theme* are included as dependencies. While these are also included by Composer, it's necessary to also include them here, because their assets will be accessed by npm and Webpack Encore commands, and npm commands, written in JavaScript, can't access Composer PHP dependencies. The dev dependencies include, among others, Webpack Encore. Of course, each of these dependencies also has its own "sub-dependencies", just like with Composer. All of these dependencies will be installed when running `npm install`, `php bakery assets:build` or `php bakery assets:install`. Finally, the *scripts* section exposes the Encore build tasks.

Dependencies installation will be automatically handled by the `php bakery bake` or `php bakery assets:build` commands. Alternatively, you can still use the typical npm commands from the command line:
...
The same goes for installing your own dependencies, which should be done using npm commands directly. For example:
...
This is a list of dependent sprinkles entry files list, plus our own list. Every time you add a new sprinkle, you might need to add its `webpack.entries` to this list. Also notice how the `.js` extension of the actual file reference is omitted, which is totally fine as it's a feature of Node.js.
