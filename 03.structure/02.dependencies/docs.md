...
Except for the Bakery system (which uses _[Symfony Console](#Symphony-Console-5)_, more on that later), UserFrosting uses Slim at every level to perform middleware management, route collections, and everything else needed to actually display a web page.
