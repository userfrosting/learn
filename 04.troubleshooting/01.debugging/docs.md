...
>>>> Some runtime errors cannot be caught by the error-handling system - for example, PHP fatal errors and syntax errors.  If these occur, you will only see a generic, plainly formatted "server error" page.  In this case, the actual error message will be logged with PHP's [native error logging](http://php.net/manual/en/book.errorfunc.php).  You should make sure that PHP's `log_errors` directive is set to `on`, and that you have specified a path to an error log file (what we will call the **php error log**) with the `error_log` directive.  Enabling `log_errors` and disabling `display_errors` will cause PHP to dump error messages to the log instead of the response.
